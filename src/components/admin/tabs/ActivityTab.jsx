import { Activity, Heart, MessageSquare, Trash2 } from "lucide-react";
          ...
          <div className="divide-y divide-slate-100">
            {filteredActivity.map((item) => (
              <div key={item._id || item.id} className="flex items-start gap-4 p-4 transition-colors hover:bg-slate-50">
                <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${item.type === "comment" ? "bg-blue-100" : "bg-rose-100"}`}>
                  {item.type === "comment" ? <MessageSquare className="h-5 w-5 text-blue-600" /> : <Heart className="h-5 w-5 text-rose-600" />}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-900">@{item.actorName}</span>
                    <span className="text-slate-500">{item.type === "comment" ? "commented on" : "liked"}</span>
