  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4">
        ...
        <select value={paperStatusFilter} onChange={(e) => setPaperStatusFilter(e.target.value)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700">
          <option value="all">All Status</option>
          <option value="approved">Approved</option>
          <option value="pending">Pending</option>
          <option value="rejected">Rejected</option>
        </select>
        <select value={paperDeptFilter} onChange={(e) => setPaperDeptFilter(e.target.value)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700">
          ...
          <div className="max-h-[600px] space-y-2 overflow-y-auto pr-2">
            {filteredPapers.length === 0 ? (
              ...
              filteredPapers.map((paper) => (
                <div key={paper._id} className={`flex items-center justify-between gap-3 rounded-xl p-3 transition-colors ${editingPaperId === paper._id ? "bg-blue-50 ring-2 ring-blue-200" : "bg-slate-50 hover:bg-slate-100"}`}>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="truncate text-sm font-semibold text-slate-900">{paper.title}</p>
                      <StatusBadge status={paper.status} />
                    </div>
                    <p className="truncate text-xs text-slate-500">{paper.department} • {paper.subject} • {paper.year}</p>
                  </div>
